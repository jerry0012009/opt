...
<head>
  ...
  <style>
    :root { color-scheme: dark; --bg:#101214; --panel:#191d21; --text:#eef2f5; --muted:#9aa6b2; --line:#2b333b; --accent:#4ea1ff; --danger:#ff6b6b; }
  ...
  <script>
    ...
    async function refresh() {
      try {
        ...
        $('status').textContent = data.running ? 'tmux: ' + data.session : 'tmux: not running';
...
async function handle(req, res) {
  ...
  try {
    if (req.method === 'GET' && pathname === `${BASE_PATH}/api/status`) {
      ...
      sendJson(res, 200, { session: SESSION, running, capture: running ? await capturePane(90) : '' });
      return;
    }
